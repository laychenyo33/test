...
//        $sql = $this->sqlstr_add_limit($op_limit,$nowp,$sql);
    ...
    function header_footer($meta_array,$seo_h1=""){
        ...
        if($e==1){  //第一次執行才做
            ...
            $tpl->assignGlobal("MSG_PRODUCT_SEARCH_INPUT",$_GET['kw']);
            $tpl->assignGlobal("HEADER_META_ROBOT",($cms_cfg['ws_online']) ? "all" : "nofollow");
                ...
                if($rsnum1 > 0 ){ //有次分類
                    ...
                    while($row1 = $db->fetch_array($selectrs1,1)){
                        ...
                        $tpl->assign( array( "VALUE_SUBCATE_NAME" => $row1["pc_name"],
                                             ...
                                             "TAG_CURRENT_CLASS"  => (($cms_cfg["ws_module"]['ws_seo'] && ($row1["pc_seo_filename"]==$_REQUEST["d"] || $row1["pc_seo_filename"]==$_REQUEST["f"])) || $_REQUEST['pc_parent']==$row1['pc_id'])?"class='current'":"",
                        ));
                        //左方產品次分類為click menu
                        if($cms_cfg['ws_module']['ws_seo']==1){
                            if($_REQUEST["d"] || $_REQUEST["f"]) {
    ...
    function mamage_authority(){
        ...
        ($_SESSION[$cms_cfg['sess_cookie_name']]["AUTHORITY"]["aa_order"] && $cms_cfg["ws_module"]["ws_cart_plus_shopping"])?$tpl->newBlock( "AUTHORITY_ORDER_PLUS_SHOPPING" ):"";
        ...
        if($_SESSION[$cms_cfg['sess_cookie_name']]["AUTHORITY"]["aa_products"] && $cms_cfg["ws_module"]["ws_new_product"]){
            ...
            $tpl->newBlock("AUTHORITY_PRO_PRODUCTS");
            $tpl->gotoBlock( "AUTHORITY_PRODUCTS" );
        }
        ...
        $replace_option = array(
            'in' => array(
                'pattern' => array(
                    ...
                    '%(https://'.$cms_cfg['server_name'].')('.$cms_cfg['base_root'].')([^\s"><]+)%i',
                    '%(https://'.$cms_cfg['server_name'].')('.$cms_cfg['file_root'].')([^\s"><]+)%i',
                    '%((http://)*'.$cms_cfg['server_name'].')('.$cms_cfg['file_root'].')((upload_files/|tiny_mce/|tinymce/)[^\s"><]+\.(html|htm|php|png|gif|jpg|jpeg|js|css))%i',
                    '%((http://)*'.$cms_cfg['server_name'].')('.$cms_cfg['base_root'].')([^\s"><]+)%i',
        ...
        if($sw){
            ...
            if(in_array('aboutus',$sw)){
                ...
                while($row=$db->fetch_array($res,1)){
                    ...
                    $menu_arr[$row['au_cate']][]=array(
                        'name'=>$row['au_subject'],
                        'link'=> ($i==1)?$cms_cfg['base_root'].$row['au_cate'].".htm" : App::getHelper('request')->get_link('aboutus',$row),
                    );
                    $i++;
                }
            }
