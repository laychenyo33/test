...
class LOGIN {
    ...
    function Check_Login_Data(){
        ...
        if (isset($_POST['callback']) && $this->valid_pass) {
            $sql="select m.m_id,m.m_account,m.m_name,mc.mc_subject,mc.mc_discount from ".$cms_cfg['tb_prefix']."_member as m left join ".$cms_cfg['tb_prefix']."_member_cate as mc on mc.mc_id=m.mc_id
            ...
            if ($rsnum > 0) {
                ...
                $_SESSION[$cms_cfg['sess_cookie_name']]["MEMBER_NAME"]=$row["m_name"];
