...
class lunarCalendar {
        ...
        protected static  $_ConstellationName =  array(
              "白羊座", "金牛座", "雙子座",
              "巨蟹座", "獅子座", "處女座",
              "天秤座", "天蠍座", "射手座",
              "摩羯座", "水瓶座", "雙魚座"
        ...
        protected static $_LunarHolDayName = array(
              "小寒", "大寒", "立春", "雨水",
              "驚蟄", "春分", "清明", "谷雨",
              "立夏", "小滿", "芒種", "夏至",
              "小暑", "大暑", "立秋", "處暑",
        ...
        protected static $_gLunarMonthDay = array(
        ...
        public  static $_gLunarMonth = array(
        ...
        public static $gLunarHoliDay = array(
        ...
        public function __construct($date=null){
            ...
            $this->_m_Date = new myDateTime($date_str,new DateTimeZone("Asia/Taipei"));
        }
...
//             $Constellation;
            ...
            { return self::$_ConstellationName[$Constellation]; }
            else
            { return ""; };
        }
        ...
        /// </summary>
        /// <returns></returns>
        public function l_GetLunarHolDay()
        {
            $Flag=null;
            $Day;$iYear;$iMonth;$iDay;
            $iYear = $this->_m_Date->Year;
            if (($iYear < self::START_YEAR) || ($iYear > self::END_YEAR))
            { return -1; }
            $iMonth = $this->_m_Date->Month;
            $iDay = $this->_m_Date->Day;
            $Flag = self::$gLunarHoliDay[($iYear - self::START_YEAR) * 12 + $iMonth - 1];
            if ($iDay < 15)
            { $Day = 15 - (($Flag >> 4) & 0x0f); }
            else
            { $Day = ($Flag & 0x0f) + 15; };
        ...
        /// <returns></returns>
        public function FormatMonth($iMonth, $bLunar)
        {
            $szText = "正二三四五六七八九十";
            $strMonth = "";
            ...
            {
                $strMonth = "";
                $strMonth = $strMonth . mb_substr($szText,$iMonth - 1, 1,"utf-8");//.Substring();
                $strMonth = $strMonth . "月";
                return $strMonth;
            }
        ...
        /// <returns></returns>
        public function FormatLunarDay($iDay)
        {
            $szText1 = "初十廿三";
            $szText2 = "一二三四五六七八九十";
            $strDay;
            if (($iDay != 20) && ($iDay != 30))
            {
                $strDay = mb_substr($szText1,($iDay - 1) / 10, 1,"utf-8");//.Substring();
                $strDay = $strDay . mb_substr($szText2,($iDay - 1) % 10, 1,"utf-8");//.Substring();
            }
            else
            {
                $strDay = mb_substr($szText1,($iDay / 10) * 2 + 1, 2,"utf-8");//.Substring();
                $strDay = $strDay . "十";
            }
            return $strDay;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public function GetLunarNewYearDate()
        {
            $iYear; $iMonth; $iDay;
            ...
            $iYear = $this->_m_Date->Year;
            /*待修改*/
            if (($iYear < self::START_YEAR) || ($iYear > self::END_YEAR)) { return 0; }//DateTime.MinValue;//minValue是什麼？
            /*待修改*/
            //兩個dateTime互減，應該是要取得兩個日期的相差天數			
            $ts = $this->_m_Date - (new DateTime(START_YEAR, 1, 1));
            $this->l_CalcLunarDate($iYear, $iMonth, $iDay, $ts->Days);
            ...
            for ($i = 1; $i < $iMonth; $i++)
            {
                $days += $this->LunarMonthDays($iYear, i);
            }

            $days += $iDay - 1;
            /*待修改*/
            return $this->_m_Date->AddDays(-$days);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public function GetNextLunarNewYearDate()
        {
            $iYear; $iMonth; $iDay;

            $ts;
            /*待修改*/
            $iYear = $this->_m_Date->AddYears(1)->Year; //明年

            if (($iYear < self::START_YEAR) || ($iYear > self::END_YEAR)) { return DateTime.MinValue; }
            /*待修改*/
            $ts = $this->_m_Date->AddYears(1) - (new DateTime(self::START_YEAR, 1, 1));
            /*待修改*/
            $this->l_CalcLunarDate($iYear, $iMonth, $iDay, $ts->Days);
            // 今年過年至今的農曆天數
            $days = 0;
            ...
            return $this->_m_Date->AddYears(1)->AddDays(-$days);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public function GetLunarDate($iMonth, $iDay)
        {
            $iYear;

            $ts;
            $iYear = $this->m_Date->Year;
            ...
            $days += $iDay - 1;
            /*待修改*/
            $ThisNewYearDate = $this->GetLunarNewYearDate();
            /*待修改*/
        ...
        /// </summary>
        /// <returns></returns>
        public function GetLunarHolDay()
        {
            $iYear; $iMonth; $iDay;
            $i;
            $ts;
            ...
            if (($iYear < self::START_YEAR) || ($iYear > self::END_YEAR))
            { return ""; };
            $i = $this->l_GetLunarHolDay();
            if (($i >= 0) && ($i <= 23))
            { return self::$_LunarHolDayName[i]; }
            else
            {
                $ts = $this->_m_Date - (new DateTime(self::START_YEAR, 1, 1));
                $iDay = $this->l_CalcLunarDate($iYear, $iMonth, $iDay, $ts->Days);
                return $this->FormatMonth($iMonth, true) . $this->FormatLunarDay($iDay);
            }
        }
        ...
        /// <returns></returns>
        public function GetLeapMonth($iLunarYear)
        {
            $Flag;
            if (($iLunarYear < self::START_YEAR) || ($iLunarYear > self::END_YEAR))
            { return 0; };
            $Flag = self::$_gLunarMonth[($iLunarYear - self::START_YEAR) / 2];
            if (($iLunarYear - self::START_YEAR) % 2 == 0)
            { return $Flag >> 4; }
            else
            { return $Flag & 0x0F; }
        }
        ...
        /// <returns></returns>
        public function LunarMonthDays($iLunarYear, $iLunarMonth)
        {
            $Height; $Low;
            $iBit;
            if (($iLunarYear < self::START_YEAR) || ($iLunarYear > self::END_YEAR))
            ...
            $Low = 29;
            $iBit = 16 - $iLunarMonth;
            if (($iLunarMonth > $this->GetLeapMonth($iLunarYear)) && ($this->GetLeapMonth($iLunarYear) > 0))
            {
                $iBit--;
            }
            if ((self::$_gLunarMonthDay[$iLunarYear - self::START_YEAR] & (1 << $iBit)) > 0)
            {
                $Low++;
            }
            if ($iLunarMonth == $this->GetLeapMonth($iLunarYear))
            {
                if ((self::$_gLunarMonthDay[$iLunarYear - self::START_YEAR] & (1 << ($iBit - 1))) > 0)
                { $Height = 30; }
                else
                { $Height = 29; }
            }
            return $Low | $Height << 16; //合成為uint
        }
        ...
        /// <returns></returns>
        public function LunarYearDays($iLunarYear)
        {
            $Days;
            $tmp;
            if (($iLunarYear < self::START_YEAR) || ($iLunarYear > self::END_YEAR))
        ...
        public function l_CalcLunarDate($iYear, $iMonth, $iDay, $iSpanDays)
        {
            $tmp;
            ...
            $iYear = self::START_YEAR;
            $iMonth = 1;
            $iDay = 1;
            //計算年
            $tmp = $this->LunarYearDays($iYear);
            while ($iSpanDays >= $tmp)
            {
            ...
            //計算月
            $tmp = $this->LunarMonthDays($iYear, $iMonth); //取低位
            while ($iSpanDays >= $tmp)
            {
                $iSpanDays = $iSpanDays - $tmp;
                if ($iMonth == $this->GetLeapMonth($iYear))
                {
                    $tmp = ($this->LunarMonthDays($iYear, $iMonth) >> 16) & 0xFFFF; //取高位
                    if ($iSpanDays < $tmp)
                    { break; }
                    $iSpanDays = $iSpanDays - $tmp;
                }
        ...
        /// </summary>
        /// <returns></returns>
        public function FormatLunarYear()
        {
            $strYear;
            $szText1 = "甲乙丙丁戊己庚辛壬癸";
            ...
            $iYear;
            $iYear = $this->_m_Date->Year;
            $strYear = mb_substr($szText1,($iYear - 4) % 10, 1,"utf-8");
            $strYear .= mb_substr($szText2,($iYear - 4) % 12, 1,"utf-8");
            $strYear .= " ";
            $strYear .= szText3.Substring((iYear - 4) % 12, 1);
            $strYear .= "年";
            return $strYear;
        }
}
...
class myDateTime extends DateTime{
    ...
    public function __get($name){
        if(isset($this->_data[strtolower($name)])){
            ...
            throw(new Exception("no property in this class"));
        }
    }
    ...
    public function __construct($date_str="now", DateTimeZone $tz){
        parent::__construct($date_str,$tz);
        $ts = strtotime($date_str);
        $this->_data['year'] = date("Y",$ts);
        $this->_data['month'] = date("n",$ts);
        $this->_data['day'] = date("j",$ts);
    }
	
    public function AddDay($nums_of_day){
    }
}
