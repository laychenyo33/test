<?php
//MAIN MESSAGE
$TPLMSG['HOME'] = "首頁";
$TPLMSG['LANG_CHT'] = "繁體中文版";
$TPLMSG['LANG_CHS'] = "簡體中文版";
$TPLMSG['LANG_ENG'] = "英文版";
$TPLMSG['LANG_JAP'] = "日文版";
$TPLMSG['LANG_SPA'] = "西班牙文版";
$TPLMSG['LANG_FRE'] = "法文版";
$TPLMSG['LANG_GER'] = "德文版";
$TPLMSG['ALL'] = "全部";
$TPLMSG['FORM'] = "表單";
$TPLMSG['SUBJECT'] = "標題";
$TPLMSG['CONTENT'] = "內容";
$TPLMSG['NAME'] = "名稱";
$TPLMSG['JOIN'] = "加入";
$TPLMSG['INQUIRY'] = "詢問";
$TPLMSG['INQUIRY_CART'] = "詢價車";
$TPLMSG['INQUIRY_PRICE'] = "詢價";
$TPLMSG['SHOPPING'] = "購買";
$TPLMSG['SHOPPING_CART'] = "購物車";
$TPLMSG['STATUS'] = "狀態";
$TPLMSG['ADD'] = "新增";
$TPLMSG['MODIFY'] = "修改";
$TPLMSG['SEND'] = "送出";
$TPLMSG['REPLY'] = "回覆";
$TPLMSG['REPLY_YES'] = "<font color='blue'>已回覆</font>";
$TPLMSG['REPLY_NO'] = "<font color='red'>未回覆</font>";
$TPLMSG['DEL'] = "刪除";
$TPLMSG['CATE'] = "分類";
$TPLMSG['YES'] = "是";
$TPLMSG['NO'] = "否";
$TPLMSG['DATE_ONLINE'] = "上線日期";
$TPLMSG['CONTACT'] = "聯絡人";
$TPLMSG['URL'] = "網址";
$TPLMSG['OK'] = "確定";
$TPLMSG['CANCEL'] = "取消";
$TPLMSG['DATE'] = "日期";
$TPLMSG['TIME'] = "時間";
$TPLMSG['CREATEDATE'] = "建立日期";
$TPLMSG['MODIFYDATE'] = "修改日期";
$TPLMSG['STARTDATE'] = "開始日期";
$TPLMSG['ENDDATE'] = "結束日期";
$TPLMSG['TOTAL'] = "共計";
$TPLMSG['PAGES'] = "頁";
$TPLMSG['DELETE_CHECK_STR'] = "您確定要刪除嗎?";
$TPLMSG['TOTAL_RECORDS'] = "總筆數";
$TPLMSG['TOTAL_PAGES'] = "總頁數";
$TPLMSG['AMOUNT'] = "數量";
$TPLMSG['DETAIL'] = "細項";
$TPLMSG['VIEWS'] = "檢視";
$TPLMSG['FILE'] = "檔案";
$TPLMSG['PUBLISH_ON'] = "刊登中";
$TPLMSG['PUBLISH_OFF'] = "已過期";
$TPLMSG['PAGE_BACK'] = "回上頁";
$TPLMSG['EMAIL'] = "E-mail";
$TPLMSG['CLEAN'] = "清除";
$TPLMSG['COLLECTION'] = "收藏";
$TPLMSG['FAVORITE'] = "我的最愛";
$TPLMSG['DEL_CART_ITEM'] = "刪除項目";
$TPLMSG['SURE_TO_DELETE'] = "確定刪除?";
$TPLMSG['AGREE'] = "同意";
$TPLMSG['DISAGREE'] = "不同意";
//CONFIG
$TPLMSG["CUSTOM_INDEX_PAGE"]="index.html";
$TPLMSG["SYSTEM_INDEX_PAGE"]="系統預設首頁";
$TPLMSG['CHANGE_PRICE_STATUS_CLICK_ME']="按我修改";
//LOGIN
$TPLMSG['LOGIN_ACCOUNT'] = "帳號";
$TPLMSG['LOGIN_PASSWORD'] = "密碼";
$TPLMSG['LOGIN_SECURITY'] = "驗証";
$TPLMSG['LOGIN_LANGUAGE'] = "語言版本";
$TPLMSG['LOGIN_NOTICE1'] = "請先登入您的會員帳號與密碼!!";
$TPLMSG['LOGIN_FORGOT_PASSWORD'] = "忘記密碼?";
$TPLMSG['LOGIN_REGISTER'] = "加入會員";
$TPLMSG["LOGIN_BUTTON"] = "登入";
//NEWS
$TPLMSG['NEWS'] = "最新消息";
//FAQ
$TPLMSG['FAQ'] = "FAQ問與答";
//ABOUT US
$TPLMSG['ABOUT_US'] = "關於我們";
//AD
$TPLMSG['AD_UP_BANNER_580_120'] = "上方橫幅廣告 寬580 X 高120";
$TPLMSG['AD_SIDE_BUTTON_150_150'] = "側邊廣告 寬150 X 高150";
$TPLMSG['AD_SIDE_BUTTON_150_50'] = "側邊廣告 寬150 X 高50";
$TPLMSG['AD_INDEX_BANNER'] = "首頁Banner";
$TPLMSG['AD_INSIDE_BANNER'] = "內頁Banner";
$TPLMSG['AD_INSIDE_LEFT'] = "內頁左側廣告";
$TPLMSG['AD_INSIDE_RIGHT'] = "內頁右側廣告";
$TPLMSG['AD_PRODUCTS'] = "產品區廣告";
$TPLMSG['AD_INDEX_POPUP'] = "首頁彈出式廣告";
//ADMIN
$TPLMSG['ADMIN_VALID_ACCOUNT'] = "檢測帳號";
$TPLMSG['ADMIN_ACCOUNT_EXIST'] = "帳號重覆";
$TPLMSG['ADMIN_ACCOUNT_NO_USE'] = "此帳號可以使用";
$TPLMSG['ADMIN_PLEASE_INPUT_ACCOUNT'] = "請輸入帳號";
//CONTACT US
$TPLMSG['CONTACT_US'] = "聯絡我們";
$TPLMSG['CONTACT_US_PRODUCTS'] = "產品相關";
$TPLMSG['CONTACT_ERROR_REPORTING'] = "錯誤回報";
$TPLMSG['CONTACT_SUGGESTION'] = "建議事項";
$TPLMSG['CONTACT_US_OTHERS'] = "其他";
$TPLMSG['CONTACT_US_MAIL_NOTICE'] = "感謝您的建議及提問，我們會儘快給您答覆!!";
$TPLMSG["CONTACT_US_DETAIL"]="聯絡我們詳細資料";
$TPLMSG["CONTACT_US_REPLY"]="回覆資料";
$TPLMSG["CONTACT_US_REPLY_CONTENT"]="回覆內容";
$TPLMSG["CONTACT_US_REPLY_TIME"]="回覆時間";
$TPLMSG['CONTACT_US_COMPANY_NAME']="公司名稱";
$TPLMSG['CONTACT_US_TITLE']="標 題";
$TPLMSG['CONTACT_US_FAX']="傳真";
$TPLMSG['CONTACT_US_MOBIL']="行動電話";
$TPLMSG['CONTACT_US_SITE']="企業網站";
$TPLMSG['CONTACT_US_COUNTRY']="國家";
$TPLMSG['CONTACT_US_CITY']="城市";
$TPLMSG['CONTACT_US_SUBJECT']="專案名稱";
$TPLMSG['CONTACT_US_MAIL_TOP_MESSAGE']="感謝您的來信，您的問題我們會儘快以 Email方式回覆，再次感謝您對%s的支持～";
$TPLMSG['CONTACT_US_REQUIRED_DESC']="標註(<span style=\"color:#F00; font-weight:bold\"> * </span>)的欄位為必填, 請勿留空!";
$TPLMSG['CONTACT_US_CONTENT_DESC']="請留下您的訊息 :";

//EPAPER
$TPLMSG['SUBSCRIBE'] = "訂閱電子報";
$TPLMSG['EPAPER'] = "電子報";
//MEMBER
$TPLMSG['MEMBER_ZONE']="會員專區";
$TPLMSG['BIRTHDAY']="生日";
$TPLMSG['ZIP']="郵遞區號";
$TPLMSG['SEX']="性別";
$TPLMSG['MALE']="男";
$TPLMSG['FEMALE']="女";
$TPLMSG['ADDRESS']="住址";
$TPLMSG['TEL']="電話";
$TPLMSG['FAX']="傳真";
$TPLMSG['CONTACT_PERSON']="聯絡人";
$TPLMSG['COMPANY_NAME']="公司名稱";
$TPLMSG['CELLPHONE']="行動電話";
$TPLMSG['COUNTRY']="國家";
$TPLMSG['MEMBER_CHECK_ACCOUNT']="檢查帳號";
$TPLMSG['MEMBER_CHECK_PASSWORD']="驗証密碼";
$TPLMSG['MEMBER_DATA_MOD']="會員資料修改";
$TPLMSG['MEMBER_ACCOUNT_NOT_EXISTS']="無此帳號!!";
$TPLMSG['MEMBER_LOGIN']="會員登入";
$TPLMSG["MEMBER_LOGOUT"]="會員登出";
$TPLMSG['MEMBER_JOIN']="加入會員";
$TPLMSG['MEMBER_CONFIRM_MAIL']="會員確認信";
$TPLMSG['MEMBER_NAME']="姓名";
$TPLMSG['MEMBER_FNAME']="名";
$TPLMSG['MEMBER_LNAME']="姓";
$TPLMSG['MEMBER_ZONE_DATA']="會員資料管理";
$TPLMSG['MEMBER_ZONE_ORDER']="會員訂單管理";
$TPLMSG['MEMBER_ZONE_INQUIRY']="會員詢問信管理";
$TPLMSG['MEMBER_ZONE_CONTACTUS']="聯絡我們管理";
$TPLMSG['MEMBER_ZONE_GREETINGS']="您好, %s";
$TPLMSG['MEMBER_ACCEPT_SERVICE_TERM']="接受會員條款";
$TPLMSG['MEMBER_DISCOUNT']="折扣率";
$TPLMSG['MEMBER_MUTI_TEL_NOTICE']="多個電話請用,隔開";
$TPLMSG['MEMBER_MUTI_FAX_NOTICE']="多個傳真請用,隔開";
$TPLMSG['MEMBER_MUTI_CELLPHONE_NOTICE']="多個行動電話請用,隔開";
$TPLMSG['MEMBER_MUTI_EMAIL_NOTICE']="多個e-mail請用,隔開";
$TPLMSG['MEMBER_ACCEPT_SERVICE_TERM']="接受會員條款";
$TPLMSG['MEMBER_FORGET_TITLE']="會員密碼";
$TPLMSG['MEMBER_FORGET_ACCOUNT']="帳號";
$TPLMSG['MEMBER_FORGET_PASSWORD']="密碼";
$TPLMSG['MEMBER_JOIN_VALIDATE_EMAIL'] = "請點擊以下連結回到網站驗證E-mail並啟用您的帳號:<br/>";
$TPLMSG['MEMBER_JOIN_VALIDATE_MANUAL'] = "您註冊的帳號將會有專人審核，待通過之後會寄發通知信給您，請耐心等候!";
$TPLMSG['MEMBER_APPROV_NOTIFICATION_SUBJECT'] = "您在 %s 的帳號已啟用";
$TPLMSG['MEMBER_APPROV_NOTIFICATION_CONTENT'] = "您在 %s 的帳號 %s 已啟用，歡迎回到網站上登入";
$TPLMSG['MEMBER_NAME_SET_1'] = "%1\$s%2\$s";
$TPLMSG['MEMBER_NAME_SET_2'] = "%2\$s%1\$s";
$TPLMSG["MEMBER_EPAPER"] = "願意接收電子報訊息?";
$TPLMSG["MEMBER_NAME_IN_CART_TEMPLATE"] = '%2$s%1$s';
$TPLMSG["MEMBER_ACCOUNT_EXISTS"] = "會員帳號已存在!";
$TPLMSG['MEMBER_FOOTPRINT'] = "我的足跡";
$TPLMSG['MEMBER_COLLECT'] = "我的收藏";
$TPLMSG['MEMBER_LOGIN_MESSAGE1'] = "請按此填寫資料，即可加入會員";
$TPLMSG['MEMBER_LOGIN_MESSAGE2'] = "※如您已經是會員，請從左邊選項登入";

//PRODUCTS
$TPLMSG['PRODUCTS'] = "產品介紹";
$TPLMSG['PRODUCT'] = "產品";
$TPLMSG['PRODUCT_IMG'] = "產品圖";
$TPLMSG['PRODUCT_CATE'] = "產品分類";
$TPLMSG['PRODUCT_LIST'] = "產品列表";
$TPLMSG['PRODUCT_SHOW'] = "產品展示";
$TPLMSG['PRODUCT_NAME'] = "產品名稱";
$TPLMSG['PRODUCT_NEW'] = "最新產品";
$TPLMSG['PRODUCT_HOT'] = "熱門產品";
$TPLMSG['PRODUCT_PROMOTION'] = "促銷產品";
$TPLMSG['PRODUCT_SP_SHOPPING_NOTICE'] = "目前系統功能設定為<font color='blue'>【購物車】</font>,非會員會顯示【特價】;會員登入後,會顯示會員適用的【折扣價】,<BR>如果要變更網站價格的顯示方式";
$TPLMSG['PRODUCT_SP_INQUIRY_NOTICE'] = "目前系統功能設定為<font color='blue'>【詢價車】</font>,底下所有的產品價格均會顯示【詢價】,如果要變更網站價格的顯示方式";
$TPLMSG['PRODUCT_SP_DISPLAY_NOTICE'] = "目前系統功能設定為<font color='blue'>【一般產品展示】</font>,只單純做產品展示,如果要變更網站價格的顯示方式";
$TPLMSG['PRODUCT_DESCRIPTION'] = "產品敘述";
$TPLMSG['PRODUCT_SPEC'] = "產品規格";
$TPLMSG['PRODUCT_CHARACTER'] = "產品特性";
$TPLMSG['PRODUCT_PRICE'] = "產品售價";
$TPLMSG['PRODUCT_LIST_PRICE'] = "定價";
$TPLMSG['PRODUCT_SPECIAL_PRICE'] = "特價";
$TPLMSG['PRODUCT_DISCOUNT_PRICE'] = "折扣價";
$TPLMSG['PRODUCT_SERIAL'] = "產品型號";
$TPLMSG['PRODUCT_COLLECTION'] = "我要收藏";
$TPLMSG['PRODUCT_OUT_OF_STOCK'] = "無現貨";
// CART
$TPLMSG['CART_ADD'] = "加入";
$TPLMSG['CART_INQUIRY'] = "詢價車";
$TPLMSG['CART_SHOPPING'] = "購物車";
$TPLMSG['CART_TOTAL'] = "合計";
$TPLMSG['CART_SUBTOTAL'] = "小計";
$TPLMSG['CART_AMOUNT'] = "數量";
$TPLMSG['CART_MODIFY_AMOUNT'] = "修改數量";
$TPLMSG['CART_CONTINUE_SHOPPING'] = "繼續購物";
$TPLMSG['CART_CONTINUE_INQUIRY'] = "繼續詢價";
$TPLMSG['CART_FINISH_SHOPPING'] = "我要結帳";
$TPLMSG['CART_FINISH_INQUIRY'] = "送出詢價單";
$TPLMSG['CART_DELETE_CHECK'] = "確定要刪除?";
$TPLMSG['CART_EMPTY'] = "目前購物車是空的!";
$TPLMSG['CART_SPEC_TITLE'] = "規格";
$TPLMSG['CART_STEP_NEXT'] = "下一步";
$TPLMSG['CART_LOGIN_MESSAGE1'] = "請按此快速結帳，並註冊為會員";
$TPLMSG['CART_LOGIN_MESSAGE2'] = "※如您已經是會員，請從左邊選項登入";
$TPLMSG['SHIPPING_PRICE'] = "運費";

//SERVICE TERM
$TPLMSG["SERVICE_CENTER"] = "服務中心";
$TPLMSG["CONTACTUS_TERM"] = "聯絡我們說明";
$TPLMSG["JOIN_MEMBER_MAIL"] = "會員加入自動回覆通知設定";
$TPLMSG["PAYMENT_TERM"] = "付款說明";
$TPLMSG["PRIVACY_POLICY"] = "隱私權政策";
$TPLMSG["SERVICE_TERM"] = "會員服務條款";
$TPLMSG["SHIPPING_TERM"] = "運送說明";
$TPLMSG["SHOPPING_TERM"] = "購物說明";
$TPLMSG["INQUIRY_MAIL"] = "詢問車自動回覆通知設定";
$TPLMSG["ORDER_MAIL"] = "購物車自動回覆通知設定";
$TPLMSG["INQUIRY_MAIL_TITLE"] = "詢問確認信";
$TPLMSG["ORDER_MAIL_TITLE"] = "訂單確認信";
$TPLMSG["ORDER_RECEIPT_NOTIFY_TITLE"] = "繳款通知信";

//ORDER
$TPLMSG["ORDER_ID"]="訂單編號";
$TPLMSG["ORDER_TOTAL_MONEY"]="訂單金額";
$TPLMSG["ORDER_NEW"]="新訂單";
$TPLMSG["ORDER_DEALING"]="處理中";
$TPLMSG["ORDER_PRODUCTS_SEND"]="出貨中";
$TPLMSG["ORDER_COMPLETED"]="已完成";
$TPLMSG["ORDER_CANCEL"]="<font color='red'>取消訂單</font>";
$TPLMSG["ORDER_REJECT"]="<font color='red'>拒絕訂單</font>";
$TPLMSG["ORDER_DETAIL"]="產品訂單詳細資料";
$TPLMSG["ORDER_CONTENT"]="訂單內容";
$TPLMSG["ORDER_UNPAID"]="未付款";
$TPLMSG["ORDER_PAID"]="已付款";
$TPLMSG['ORDER_SHIP_ZONE'] = "送貨區域";
$TPLMSG['ORDER_BLOCK_TITLE_ORDER'] = "訂單資訊";
$TPLMSG['ORDER_BLOCK_TITLE_ORDERBY'] = "訂購者資訊";
$TPLMSG['ORDER_BLOCK_TITLE_SENDTO'] = "收件者資訊";
$TPLMSG['ORDER_BLOCK_TITLE_VAT_RECEIPT'] = "發票資訊";
$TPLMSG['ORDER_PREVIEW_MODIFY'] = "修改資料";
$TPLMSG['ORDER_PREVIEW_FINISH'] = "完成購物";
$TPLMSG['ORDER_DATA_SHORTAGE'] = "資料短缺，請重新填寫!";

//PAYMENT
$TPLMSG["PAYMENT_TYPE"]="付款方式";
$TPLMSG["PAYMENT_ATM"]="ATM轉帳或銀行匯款";
$TPLMSG["PAYMENT_CASH_ON_DELIVERY"]="貨到付款";
$TPLMSG['CHOOSE_PAYMENT_TYPE'] = "請選擇付款方式";

//INQUIRY
$TPLMSG["INQUIRY_ID"]="詢問信編號";
$TPLMSG["INQUIRY_DETAIL"]="產品詢問詳細資料";
$TPLMSG["INQUIRY_CONTENT"]="詢問內容";
$TPLMSG["INQUIRY_QUANTITY_UPDATED"]="已更新數量!";

//DOWNLOAD
$TPLMSG["DOWNLOAD"]="檔案分享";
$TPLMSG['DOWNLOAD_AFTER_READ'] = "讀取下載(不顯示真實路徑)";
$TPLMSG['DOWNLOAD_DIRECTLY'] = "直接下載(顯示真實路徑)";
        
//FORGOT PASSWORD
$TPLMSG["FORGOT_PASSWORD"]="忘記密碼";

//EBOOK
$TPLMSG["EBOOK"]="電子型錄";
$TPLMSG["EBOOK_DOWNLOAD"]="下載";
$TPLMSG["EBOOK_PRINT"] ="列印";

//ACTION MESSAGE
$TPLMSG['ACTION_TERM'] = "資料已處理完成<BR>請靜待2秒回到列表頁面!!";
$TPLMSG['ACTION_TERM_JS'] = "資料已送出!!";
$TPLMSG["ACCOUNT_EXIST"] ="此帳號已存在,請重新輸入";
$TPLMSG["CREATE_CATE_FIRST"] = "請先建立分類!!<BR>請靜待2秒回到新增分類頁面!!";
$TPLMSG["DIR_NOT_EMPTY"] = "資料夾尚有資料,請先刪除";
//ERROR MESSAGE
$TPLMSG['LOGIN_ERROR'] = "請輸入正確的帳號密碼!!";
$TPLMSG['SECURITY_ERROR'] = "驗証碼錯誤!!";
//DATA MESSAGE
$TPLMSG['NO_DATA'] = "查無資料!!";
$TPLMSG['NO_CATE'] = "未分類";
//JS MESSAGE
$TPLMSG["CONFIRM_PASSWORD"]="確認密碼";
//SITE MAP
$TPLMSG["SITEMAP"]="網站地圖";

//DELIVERY_NOTICE
$TPLMSG['DELIVERY_NOTICE'] = "出貨通知";
$TPLMSG['DELIVERY_ID'] = "訂單編號";
$TPLMSG['DELIVERY_TOTAL_PRICE'] = "出貨金額";
$TPLMSG['DELIVERY_DATE'] = "出貨日期";

//VIRTUAL ACCOUNT
$TPLMSG['TRANSFER_BANK'] = "轉帳銀行";
$TPLMSG['TRANSFER_BANK_CODE'] = "台灣銀行 (代碼：004)";
$TPLMSG['TRANSFER_ACCOUNT'] = "轉帳帳號";

$TPLMSG['SELECT_COUNTRY'] = "選擇國家";

$TPLMSG['EPAPER_REGISTERED'] = "已為您提交的email訂閱電子報";
$TPLMSG['EPAPER_EMAIL_EXISTED'] = "E-mail已在電子報發送名單!";
$TPLMSG['EPAPER_MISSING_DATA'] = "E-mail資料不存在!";
$TPLMSG['EPAPER_EMAIL_INVALID'] = "E-mail無效!";
$TPLMSG['EPAPER_SENT'] = "電子報發送完成";

$TPLMSG['SLIDE_BOX_IMAGE'] = "圖片";
$TPLMSG['SLIDE_BOX_OF'] = "之";
$TPLMSG['SLIDE_BOX_BTN_BACK'] = "關閉";
$TPLMSG['SLIDE_BOX_BTN_NEXT'] = "下一張";
$TPLMSG['SLIDE_BOX_BTN_PREV'] = "前一張";
$TPLMSG['SLIDE_BOX_CLICK_VIEW_LIST'] = "顯示圖片清單";
$TPLMSG['SLIDE_BOX_CLICK_CLOSE_LIST'] = "關閉圖片清單";

$TPLMSG['SUBMIT'] = "提交";
$TPLMSG['COMPANY_ADDRESS'] = "地址";
$TPLMSG['PRODUCTS_FAVORITE'] = "加為最愛";

$TPLMSG["APPLICATION"] = "應用領域";
$TPLMSG['PRODUCTS_SEARCH'] = "搜尋產品";
$TPLMSG['ENTER_KEYWORD'] = "輸入關鍵字";
$TPLMSG['NEXT'] = "下一筆";
$TPLMSG['PREV'] = "上一筆";
$TPLMSG['BACK_TO_LIST']="回到列表";
$TPLMSG['RELATED_PRODUCTS'] = "相關產品";
$TPLMSG["VIDEO"] = "影片";
$TPLMSG["ABOUT_US_CATE_DESC"] = "Company Information";
$TPLMSG["PRODUCTS_CATE_DESC"] = "Products Catalogue";
$TPLMSG["APPLICATION_CATE_DESC"] = "Products Appliction";
$TPLMSG["DOWNLOAD_CATE_DESC"] = "download";
$TPLMSG["NEWS_CATE_DESC"] = "Our News & Events";
$TPLMSG["VIDEO_CATE_DESC"] = "video";
$TPLMSG["FAQ_CATE_DESC"] = "faq";
$TPLMSG["SERVICE_CATE_DESC"] = "service";
$TPLMSG["CONTACT_US_CATE_DESC"] = "contact us";
$TPLMSG["SITEMAP_CATE_DESC"] = "sitemap";
$TPLMSG['CONTACT_US_ATTACH_FILES'] = "上傳檔案";
$TPLMSG["STORES"] = "門市介紹";
$TPLMSG["STORES_CATE_DESC"] = "門市介紹";
$TPLMSG['GUESTBOOK'] = "線上留言";
$TPLMSG['GUESTBOOK_CATE_DESC'] = "線上留言";
$TPLMSG["FACTORY"] = "觀光工廠";
$TPLMSG['EBOOK_CATE_DESC'] = "電子型錄";
$TPLMSG['THUMB'] = "縮圖";
$TPLMSG['CART_QUICK_SHOPPING'] = "快速購物";
$TPLMSG['CONTACT_S_1']="先生";
$TPLMSG['CONTACT_S_2']="女士";
$TPLMSG['CONTACT_S_3']="小姐";
$TPLMSG['ACCOUNT_ACTIVATED'] = "您的帳號已啟用!!請重新登入";
$TPLMSG['ACCOUNT_ACTIVATE_FAILED'] = "帳號啟用失敗";
$TPLMSG['REGISTER_NOTIFICATION'] = "在送出註冊資訊後，系統會寄一封驗證信到您的信箱。您需要點擊其中的驗證連結才能啟用您的帳號";
$TPLMSG['CONTACTUS_PRODUCT_LIST'] = "產品清單";
$TPLMSG['CONTACTUS_POSITION'] = "部門與職稱";
$TPLMSG['EPAPER_HEADER'] = "電子報頁首";
$TPLMSG['EPAPER_FOOTER'] = "電子報頁尾";
$TPLMSG['EPAPER_UNSUB_SUCCESS'] = "您已經從郵件清單裡移除了!";
$TPLMSG['EPAPER_CUSTOM_FIELD'] = "自訂條件";

$TPLMSG['GALLERY'] = "活動剪影";
$TPLMSG["PAGE_NO_EXITS"] = "此頁面不存在";
//Goodlink
$TPLMSG['GOODLINK'] = "好站連結";
$TPLMSG['FIRST_TIME_SHOPPING'] = "第一次購物";
$TPLMSG['FIRST_S_BTN'] = "第一次購物";
$TPLMSG['FIRST_I_BTN'] = "第一次詢價";
$TPLMSG['STORE_CATE_1'] = "各地門市";
$TPLMSG['STORE_CATE_2'] = "網路商城";
$TPLMSG['MEMBER_INFO'] = "訂購人資料"; //會員資訊
$TPLMSG['ADDRESSEE'] = "收件人資料"; //收件人
$TPLMSG['SAM'] = "同訂購人資料"; //同會員資料
$TPLMSG['INVOICE_INFO'] = "發票資訊"; //發票資訊
$TPLMSG['DUP_INVOICE'] = "二聯式發票"; //二聯式發票
$TPLMSG['TRI_INVOICE'] = "三聯式發票"; //三聯式發票
$TPLMSG['DONATE'] = "捐贈"; //捐贈
$TPLMSG['VAT'] = "統一編號"; //統一編號
$TPLMSG['ARRIVAL_TIME'] = "到貨時間"; //到貨時間
$TPLMSG['YEAR'] = "年"; //年
$TPLMSG['MONTH'] = "月"; //月
$TPLMSG['DAY'] = "日"; //日
$TPLMSG['NO_PAYMENT'] = "請選擇付款方式"; //請選擇付款方式
$TPLMSG["PLUS_FEE"] = "手續費"; //手續費
$TPLMSG["SHOPPING_RESULT"] = "訂單結果"; 
$TPLMSG['AUTHORIZE_TERMINATE'] = "授權中斷";
$TPLMSG['AUTHORIZE_FAILED'] = "授權失敗";
$TPLMSG['AUTHOZIED_FAILED_EXTEND_MSG'] = "<div class='auth_err_rc'>%s</div><div class='auth_err_msg'>請確認您的信用卡資訊是否正確！<br/>如果您確定輸入是正確的資訊，請與您的發卡銀行聯絡。<br/>若造成您的不便，敬請見諒。 </div>";
$TPLMSG['ORDER_SUCCESS'] = "訂購成功";
$TPLMSG['ORDER_FAIL'] = "訂購失敗";

//歐付寶相關字彙
$TPLMSG["CREDIT"] = "信用卡"; //信用卡
$TPLMSG["WEBATM"] = "網路ATM"; //網路ATM
$TPLMSG["ATM"] = "ATM轉帳"; //自動櫃員機
$TPLMSG["CVS"] = "超商代碼"; //超商代碼 (需加收 30 元手續費)
$TPLMSG["BARCODE"] = "超商條碼"; //超商條碼 (需加收 30 元手續費)
$TPLMSG["ALIPAY"] = "Alipay"; //支付寶
$TPLMSG["TENPAY"] = "Tenpay"; //財付通
$TPLMSG["TOPUPUSED"] = "Top-up"; //儲值消費
$TPLMSG['ALLPAY_BANK_CODE'] = "ATM Bank Code"; //ATM 銀行代碼
$TPLMSG['ALLPAY_VACCOUNT'] = "ATM Premium Account"; //ATM 付費帳號
$TPLMSG['ALLPAY_CVS_NO'] = "CVS code"; //超商繳費代碼
$TPLMSG['ALLPAY_CVS_BAR_1'] = "Barcode 1"; //超商繳費條碼 1
$TPLMSG['ALLPAY_CVS_BAR_2'] = "Barcode 2"; //超商繳費條碼 2
$TPLMSG['ALLPAY_CVS_BAR_3'] = "Barcode 3"; //超商繳費條碼 3
$TPLMSG['ALLPAY_EXPIRE'] = "Payment deadline"; //繳費期限
$TPLMSG['ORDER_PENDING'] = "Pending payment"; //待付款
$TPLMSG['ORDER_DONE'] = "Paid"; //已付款

/*週日名稱*/
$TPLMSG['W_SUNDAY'] = "星期日";
$TPLMSG['W_MONDAY'] = "星期一";
$TPLMSG['W_TUESDAY'] = "星期二";
$TPLMSG['W_WEDNESDAY'] = "星期三";
$TPLMSG['W_THRUSDAY'] = "星期四";
$TPLMSG['W_FRIDAY'] = "星期五";
$TPLMSG['W_SATURDAY'] = "星期六";
$TPLMSG['W_SUN'] = "日";
$TPLMSG['W_MON'] = "一";
$TPLMSG['W_TUE'] = "二";
$TPLMSG['W_WED'] = "三";
$TPLMSG['W_THR'] = "四";
$TPLMSG['W_FRI'] = "五";
$TPLMSG['W_SAT'] = "六";

/*新增語彙*/
/*以下是支付寶相關字彙*/
$TPLMSG["PAYMENT_ALI"] = "支付寶";
$TPLMSG["PAYMENT_TEN"] = "財付通";
$TPLMSG['ALI_FAIL_TITLE'] = "交易失敗";
$TPLMSG['ALI_OK_TITLE'] = "交易完成";
$TPLMSG["ALI_TITLE"] = "購物訊息";
$TPLMSG['ALI_FALE_MESSAGE'] = "產品售完或已下架";
$TPLMSG["ALI_SHIP_MSG"] = "請於支付寶平台確認運費";
$TPLMSG["MEMBER_ALI_QUERY"] = "查詢支付寶訂單";
$TPLMSG['SHIPPMENT_1'] = "臺灣本島";
$TPLMSG['SHIPPMENT_2'] = "離島地區(澎湖、金門、馬祖)";
$TPLMSG['SHIPPMENT_3'] = "中國大陸(支付寶)";
$TPLMSG['THANKS_FOR_ORDER'] = "大黑松小倆口感謝您的訂購!";
$TPLMSG['INVOICE_TYPE_2'] = "二聯式";
$TPLMSG['INVOICE_TYPE_3'] = "三聯式";
$TPLMSG['ORDER_BUYER_INFO'] = "訂購者資訊";
$TPLMSG['ORDER_RECI_INFO'] = "收件者資訊";
$TPLMSG['ORDER_INVOICE_INFO'] = "發票資訊";
$TPLMSG['DELIVER_STR'] = "配送日期及時段";
$TPLMSG['INVOICE_TYPE'] = "發票類型";
$TPLMSG['VAT_NUMBER'] = "統一編號";
$TPLMSG['PRODUCT_PRICE'] = "價格";
$TPLMSG['READ_MORE'] = "詳全文";
$TPLMSG['ORDER_MEMO'] = "消費者備註";
$TPLMSG['NEED_EXTRA_VAT_INFO'] = "請輸入公司名及統一編號";

//關閉網站
$TPLMSG['WEB_CLOSE_MSG'] = "很抱歉，我們的網站目前暫時關閉.";

$TPLMSG['ATTACH'] = "附加檔案";
$TPLMSG['MAX_UPLOAD_SIZE'] = "最大可上傳檔案大小: %s";
$TPLMSG['COMMENT_NOTIFY_SUBJECT'] = "有評論";
$TPLMSG['COMMENT_NOTIFICATION'] = '您在此<a href="%1$s">%1$s</a>有評論';
$TPLMSG['COMMENT_VALID_TAGS'] = "可用html標籤: &lt;b&gt;, &lt;p&gt;, &lt;br&gt;, &lt;a&gt;";
$TPLMSG['COMMENT_GUEST'] = "遊客";
$TPLMSG['COMMENT_ADMIN'] = "管理者";
$TPLMSG['COMMENT_ROLE_WRAPPER1'] = "<span class='role'>%s</span>";
$TPLMSG['COMMENT_ROLE_WRAPPER2'] = "<span class='role-user'>%s</span>";
$TPLMSG['COMMENT_ROLE_WRAPPER3'] = "<span class='role-admin'>%s</span>";
$TPLMSG['COMMENT_ROLE'] = "身份: ".$TPLMSG['COMMENT_ROLE_WRAPPER1'];
$TPLMSG['COMMENT_FORM_VALID_MSG'] = "請在紅框欄位填入資料";
$TPLMSG['COMMENT_UPLOAD_DESCRIPTION'] = "您可以上傳圖片，但非必要";
$TPLMSG['COMMENT_LIST_TITLE'] = "評論標題";
$TPLMSG['COMMENT_LIST_PUBLISH_DATE'] = "發布時間";

$TPLMSG['MOBILE_VERSION'] = "手機版";
$TPLMSG['COMPUTER_VERSION'] = "電腦版";
    
$TPLMSG['COPY_URL_DONE'] = "已複製網址!";

$TPLMSG['STATUS_OFF'] = "停用";
$TPLMSG['STATUS_ON'] = "啟用";
$TPLMSG['QUANTITY_DISCOUNT'] = $TPLMSG['MEMBER_DISCOUNT'];
$TPLMSG['CLASSIFY_OTHER'] = "其他";
    
//pagiation
$TPLMSG['PAGINATION_FIRST_PAGE'] = "第一頁";
$TPLMSG['PAGINATION_LAST_PAGE'] = "最後頁";
$TPLMSG['PAGINATION_PREV_FEW_PAGE'] = "前%d頁";
$TPLMSG['PAGINATION_NEXT_FEW_PAGE'] = "後%d頁";

$TPLMSG['CART_PRODUCT_NAME'] = "品名";
$TPLMSG['OPERATION'] = "操作";

$TPLMSG['PROD_TO_CART_INQUIRY'] = "詢價";
$TPLMSG['PROD_TO_CART_SHOPPING'] = "加到購物車";
$TPLMSG['ADD_TO_INQUIRY'] = "已加到詢價車";
$TPLMSG['ADD_TO_SHOPPING'] = "已加到購物車";
$TPLMSG['WARNING_ADD_NOSPEC'] = "請先選擇產品規格!";
$TPLMSG['WARNING_ADD_NOAMOUNT'] = "請先選擇購物數量!";
$TPLMSG['WARNING_AMOUNT_FORMAT'] = "輸入數值有誤!回復原值";

$TPLMSG['QUANTITY_DISCOUNT_ITEM_WRAPPER'] = '%d個以上，%d％↓';
$TPLMSG['PRODUCTS_QUANTITY_DISCOUNT_DESC'] = "本產品有數量折扣";

$TPLMSG['INVENTORY_SHORTAG_NOTIFY'] = "庫存量不足訂購數，請修改訂購量，謝謝。";
$TPLMSG['TEMP_STORE'] = "商品寄放";
$TPLMSG['SECURITY_ERROR'] = "驗證碼錯誤!!";

$TPLMSG['NO_STOCKS_NOTICE'] = "預訂請留言";
?>
