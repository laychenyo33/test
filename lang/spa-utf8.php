<?php
//MAIN MESSAGE
$TPLMSG['HOME'] = "Inicio";
$TPLMSG['LANG_CHT'] = "Chino";
$TPLMSG['LANG_ENG'] = "Inglés";
$TPLMSG['LANG_JAP'] = "Japonés";
$TPLMSG['ALL'] = "Todos los";
$TPLMSG['FORM'] = "Forma";
$TPLMSG['SUBJECT'] = "Asunto";
$TPLMSG['CONTENT'] = "Contenido";
$TPLMSG['NAME'] = "Nombre";
$TPLMSG['JOIN'] = "Únete";
$TPLMSG['INQUIRY'] = "Encuesta";
$TPLMSG['INQUIRY_CART'] = "Encuesta de compra";
$TPLMSG['INQUIRY_PRICE'] = "Encuesta";
$TPLMSG['SHOPPING'] = "Comprar";
$TPLMSG['SHOPPING_CART'] = "La cesta de compra";
$TPLMSG['STATUS'] = "Condición Jurídica y Social";
$TPLMSG['ADD'] = "Agregar";
$TPLMSG['MODIFY'] = "Modificar";
$TPLMSG['SEND'] = "Enviar";
$TPLMSG['REPLY'] = "Responder";
$TPLMSG['REPLY_YES'] = "<font color='blue'>Sí</font>";
$TPLMSG['REPLY_NO'] = "<font color='red'>No</font>";
$TPLMSG['DEL'] = "Eliminar";
$TPLMSG['CATE'] = "Categoría";
$TPLMSG['YES'] = "Sí";
$TPLMSG['NO'] = "No";
$TPLMSG['DATE_ONLINE'] = "Fecha en línea";
$TPLMSG['CONTACT'] = "Póngase en contacto con el hombre";
$TPLMSG['URL'] = "URL";
$TPLMSG['OK'] = "Aceptar";
$TPLMSG['CANCEL'] = "Cancelar";
$TPLMSG['DATE'] = "Fecha";
$TPLMSG['TIME'] = "Tiempo";
$TPLMSG['CREATEDATE'] = "Creado";
$TPLMSG['MODIFYDATE'] = "Modificado";
$TPLMSG['STARTDATE'] = "Inicio";
$TPLMSG['ENDDATE'] = "Expirará";
$TPLMSG['TOTAL'] = "Total";
$TPLMSG['PAGES'] = "Páginas";
$TPLMSG['DELETE_CHECK_STR'] = "¿Estás seguro de?";
$TPLMSG['TOTAL_RECORDS'] = "Registros en total?";
$TPLMSG['TOTAL_PAGES'] = "Total de Páginas";
$TPLMSG['AMOUNT'] = "Cantidad";
$TPLMSG['DETAIL'] = "Detalle";
$TPLMSG['VIEWS'] = "Ver";
$TPLMSG['FILE'] = "Archivo";
$TPLMSG['PUBLISH_ON'] = "Restante del Cargo";
$TPLMSG['PUBLISH_OFF'] = "Caducado";
$TPLMSG['PAGE_BACK'] = "anterior";
$TPLMSG['EMAIL'] = "E-mail";
//CONFIG
$TPLMSG["CUSTOM_INDEX_PAGE"]="index.htm";
$TPLMSG["SYSTEM_INDEX_PAGE"]="default page";
//LOGIN
$TPLMSG['LOGIN_ACCOUNT'] = "Cuenta";
$TPLMSG['LOGIN_PASSWORD'] = "Contraseña";
$TPLMSG['LOGIN_SECURITY'] = "Código";
$TPLMSG['LOGIN_LANGUAGE'] = "Idioma";
$TPLMSG['LOGIN_NOTICE1'] = "Por favor, ingresa o regístrate para descargar los videos.";
$TPLMSG['LOGIN_FORGOT_PASSWORD'] = "Has olvidado tu contraseña?";
$TPLMSG['LOGIN_REGISTER'] = "Ir a Registro";
$TPLMSG["LOGIN_BUTTON"] = "entrada";
//NEWS
$TPLMSG['NEWS'] = "Noticias";
//FAQ
$TPLMSG['FAQ'] = "Preguntas más frecuentes";
//ABOUT US
$TPLMSG['ABOUT_US'] = "Quiénes somos";
//AD
$TPLMSG['AD_UP_BANNER_580_120'] = "Banner W580 X H120";
$TPLMSG['AD_SIDE_BUTTON_150_150'] = "Button W150 X H150";
$TPLMSG['AD_SIDE_BUTTON_150_50'] = "Button W150 X H50";
//ADMIN
$TPLMSG['ADMIN_VALID_ACCOUNT'] = "Válido Cuenta";
$TPLMSG['ADMIN_ACCOUNT_EXIST'] = "Existen";
$TPLMSG['ADMIN_ACCOUNT_NO_USE'] = "No registrado";
$TPLMSG['ADMIN_PLEASE_INPUT_ACCOUNT'] = "Por favor, cuenta de entrada.";
//CONTACT US
$TPLMSG['CONTACT_US'] = "Contáctenos";
$TPLMSG['CONTACT_US_PRODUCTS'] = "Acerca de los productos";
$TPLMSG['CONTACT_ERROR_REPORTING'] = "Error de votos";
$TPLMSG['CONTACT_SUGGESTION'] = "Sugerencia";
$TPLMSG['CONTACT_US_OTHERS'] = "Otros";
$TPLMSG['CONTACT_US_MAIL_NOTICE'] = "Le damos la bienvenida y valoramos su retroalimentación sobre nuestros productos y servicios. Vamos a ser la respuesta a su pregunta al instante.";
$TPLMSG["CONTACT_US_DETAIL"]="Detalle";
$TPLMSG["CONTACT_US_REPLY"]="Responder";
$TPLMSG["CONTACT_US_REPLY_CONTENT"]="Contenido";
$TPLMSG["CONTACT_US_REPLY_TIME"]="Respondió Tiempo";
$TPLMSG['CONTACT_US_COMPANY_NAME']="Nombre de la empresa";
$TPLMSG['CONTACT_US_TITLE']="Título";
$TPLMSG['CONTACT_US_FAX']="Fax";
$TPLMSG['CONTACT_US_MOBIL']="Móvil";
$TPLMSG['CONTACT_US_SITE']="Sitio Web";
$TPLMSG['CONTACT_US_COUNTRY']="País";
$TPLMSG['CONTACT_US_CITY']="Ciudad";
$TPLMSG['CONTACT_US_SUBJECT']="Asunto";
$TPLMSG['CONTACT_US_MAIL_TOP_MESSAGE']="Thank you for your letter,  we will reply as soon as possible , thank you for your support for %s again";

//EPAPER
$TPLMSG['SUBSCRIBE'] = "Suscribirse";
$TPLMSG['EPAPER'] = "E-paper";
//MEMBER
$TPLMSG['MEMBER_ZONE']="Nombre de miembros";
$TPLMSG['BIRTHDAY']="Cumpleaños";
$TPLMSG['ZIP']="código postal";
$TPLMSG['SEX']="";
$TPLMSG['MALE']="masculina";
$TPLMSG['FEMALE']="mujeres";
$TPLMSG['ADDRESS']="Dirección";
$TPLMSG['TEL']="Teléfono";
$TPLMSG['FAX']="Fax";
$TPLMSG['CONTACT_PERSON']="Persona de contacto";
$TPLMSG['COMPANY_NAME']="Nombre de la empresa";
$TPLMSG['CELLPHONE']="Celular";
$TPLMSG['COUNTRY']="país";
$TPLMSG['MEMBER_CHECK_ACCOUNT']="Válido Cuenta";
$TPLMSG['MEMBER_CHECK_PASSWORD']="Contraseña válido";
$TPLMSG['MEMBER_DATA_MOD']="Editar los datos básicos";
$TPLMSG['MEMBER_LOGIN']="entrada";
$TPLMSG["MEMBER_LOGOUT"]="cierre de sesión";
$TPLMSG['MEMBER_JOIN']="Registrarse";
$TPLMSG['MEMBER_CONFIRM_MAIL']="Confirmar E-mail";
$TPLMSG['MEMBER_NAME']="Nombre";
$TPLMSG['MEMBER_FNAME']="first name";
$TPLMSG['MEMBER_LNAME']="last name";
$TPLMSG['MEMBER_ZONE_DATA']="Los datos miembro";
$TPLMSG['MEMBER_ZONE_ORDER']="Los pedidos miembro";
$TPLMSG['MEMBER_ZONE_INQUIRY']="Consultas miembro";
$TPLMSG['MEMBER_ZONE_CONTACTUS']="Comentarios";
$TPLMSG['MEMBER_ACCEPT_SERVICE_TERM']="Acepte El Servicio";
$TPLMSG['MEMBER_DISCOUNT']="Descuento";
$TPLMSG['MEMBER_MUTI_TEL_NOTICE']="Separados por ','";
$TPLMSG['MEMBER_MUTI_FAX_NOTICE']="Separados por ','";
$TPLMSG['MEMBER_MUTI_CELLPHONE_NOTICE']="Separados por ','";
$TPLMSG['MEMBER_MUTI_EMAIL_NOTICE']="Separados por ','";
$TPLMSG['MEMBER_ACCEPT_SERVICE_TERM']="Estoy de acuerdo";
$TPLMSG['MEMBER_FORGET_TITLE']="Contraseña miembro";
$TPLMSG['MEMBER_FORGET_ACCOUNT']="Cuenta";
$TPLMSG['MEMBER_FORGET_PASSWORD']="Contraseña";
$TPLMSG['MEMBER_JOIN_VALIDATE_EMAIL'] = "Por favor, haz clic en el siguiente enlace de vuelta al sitio para verificar el correo electrónico y activar su cuenta: <br/>";
$TPLMSG['MEMBER_JOIN_VALIDATE_MANUAL'] = "Su cuenta registrada tendrá la auditoría a alguien, que se adoptará después de la carta de notificación será enviada a usted, por favor sea paciente!";
$TPLMSG['MEMBER_NAME_SET_1'] = "%1\$s&nbsp;%2\$s";
$TPLMSG['MEMBER_NAME_SET_2'] = "%2\$s&nbsp;%1\$s";
$TPLMSG["MEMBER_EPAPER"] = "Occasionally, we email newsletters to our members. Would you like to receive this information?";
$TPLMSG["MEMBER_NAME_IN_CART_TEMPLATE"] = '%1$s %2$s';
$TPLMSG["MEMBER_ACCOUNT_EXISTS"] = "Cuenta miembro haya existido!";
$TPLMSG['MEMBER_FOOTPRINT'] = "My Footprint";
$TPLMSG['MEMBER_COLLECT'] = "My Collection";

//PRODUCTS
$TPLMSG['PRODUCTS'] = "Productos";
$TPLMSG['PRODUCT'] = "Productos";
$TPLMSG['PRODUCT_IMG'] = "Imagen del producto";
$TPLMSG['PRODUCT_CATE'] = "Categoría";
$TPLMSG['PRODUCT_LIST'] = "Lista de Productos";
$TPLMSG['PRODUCT_SHOW'] = "Display Products";
$TPLMSG['PRODUCT_NAME'] = "Nombre del producto";
$TPLMSG['PRODUCT_NEW'] = "Nuevos Productos";
$TPLMSG['PRODUCT_HOT'] = "productos calientes";
$TPLMSG['PRODUCT_PROMOTION'] = "productos de promoción";
$TPLMSG['PRODUCT_DESCRIPTION'] = "Descripción del producto";
$TPLMSG['PRODUCT_SPEC'] = "especificaciones del producto";
$TPLMSG['PRODUCT_CHARACTER'] = "producto de carácter";
$TPLMSG['PRODUCT_PRICE'] = "precio";
$TPLMSG['PRODUCT_LIST_PRICE'] = "lista de precios";
$TPLMSG['PRODUCT_SPECIAL_PRICE'] = "a la venta";
$TPLMSG['PRODUCT_DISCOUNT_PRICE'] = "descuento en el precio";
$TPLMSG['PRODUCT_SERIAL'] = "Serie del Producto";

// CART
$TPLMSG['CART_ADD'] = "";
$TPLMSG['CART_INQUIRY'] = "pregunta a la cesta";
$TPLMSG['CART_SHOPPING'] = "carro de la compra";
$TPLMSG['CART_TOTAL'] = "Total";
$TPLMSG['CART_SUBTOTAL'] = "Subtotal";
$TPLMSG['CART_AMOUNT'] = "Cantidad";
$TPLMSG['CART_MODIFY_AMOUNT'] = "Modificar Cantidad";
$TPLMSG['CART_CONTINUE_SHOPPING'] = "Continuar";
$TPLMSG['CART_CONTINUE_INQUIRY'] = "Continuar";
$TPLMSG['CART_FINISH_SHOPPING'] = "acabado";
$TPLMSG['CART_FINISH_INQUIRY'] = "acabado";
$TPLMSG['CART_DELETE_CHECK'] = "¿Está seguro de borrar los datos";
$TPLMSG['CART_EMPTY'] = "The cart is empty!";
$TPLMSG['SHIPPING_PRICE'] = "gastos de envío costos";
//SERVICE TERM
$TPLMSG["SERVICE_CENTER"] = "centro de servicio";
$TPLMSG["CONTACTUS_TERM"] = "en contacto con nosotros plazo";
$TPLMSG["JOIN_MEMBER_MAIL"] = "unirse a los miembros respuesta automática de correo electrónico";
$TPLMSG["PAYMENT_TERM"] = "término del pago";
$TPLMSG["PRIVACY_POLICY"] = "política de privacidad";
$TPLMSG["SERVICE_TERM"] = "Condiciones del servicio";
$TPLMSG["SHIPPING_TERM"] = "plazo de envío";
$TPLMSG["SHOPPING_TERM"] = "compras plazo";
$TPLMSG["INQUIRY_MAIL"] = "pregunta a la cesta respuesta automática de correo electrónico";
$TPLMSG["ORDER_MAIL"] = "carro de la compra respuesta automática de correo electrónico";
$TPLMSG["INQUIRY_MAIL_TITLE"] = "pregunta a la lista";
$TPLMSG["ORDER_MAIL_TITLE"] = "lista de compras";
$TPLMSG["ORDER_RECEIPT_NOTIFY_TITLE"] = "receipt notify";

//ORDER
$TPLMSG["ORDER_ID"]="ID del pedido";
$TPLMSG["ORDER_TOTAL_MONEY"]="Total";
$TPLMSG["ORDER_NEW"]="Nuevo";
$TPLMSG["ORDER_DEALING"]="Tratar";
$TPLMSG["ORDER_PRODUCTS_SEND"]="Preparar";
$TPLMSG["ORDER_COMPLETED"]="completado";
$TPLMSG["ORDER_CANCEL"]="<font color='red'>cancelar</font>";
$TPLMSG["ORDER_REJECT"]="<font color='red'>rechazar</font>";
$TPLMSG["ORDER_DETAIL"]="Detalle";
$TPLMSG["ORDER_CONTENT"]="Contenido";

//PAYMENT
$TPLMSG["PAYMENT_TYPE"]="tipo de pago";
$TPLMSG["PAYMENT_ATM"]="ATM";
$TPLMSG["PAYMENT_CASH_ON_DELIVERY"]="dinero en efectivo entrega";

//INQUIRY
$TPLMSG["INQUIRY_ID"]="investigación ID";
$TPLMSG["INQUIRY_DETAIL"]="Detalle";
$TPLMSG["INQUIRY_CONTENT"]="Contenido";
$TPLMSG["INQUIRY_QUANTITY_UPDATED"]="Quantity updated!";

//DOWNLOAD
$TPLMSG["DOWNLOAD"]="Descarga";

//FORGOT PASSWORD
$TPLMSG["FORGOT_PASSWORD"]="¿Has olvidado tu contraseña El";

//EBOOK
$TPLMSG["EBOOK"]="E-Catálogo";
$TPLMSG["EBOOK_DOWNLOAD"]="Download";
$TPLMSG["EBOOK_PRINT"] ="Impresión";

//ACTION MESSAGE
$TPLMSG['ACTION_TERM'] = "Un momento!!<BR>Esta página va a ser redirigido!!";
$TPLMSG['ACTION_TERM_JS'] = "sus datos han sido presentados!!";
$TPLMSG["ACCOUNT_EXIST"] ="Este es accout existido! Por favor, cuenta de entrada de nuevos!";


//ERROR MESSAGE
$TPLMSG['LOGIN_ERROR'] = "cuenta o contraseña de error!!";
$TPLMSG['SECURITY_ERROR'] = "la seguridad del código de error!!";
//DATA MESSAGE
$TPLMSG['NO_DATA'] = "vacío!!";
$TPLMSG['NO_CATE'] = "ninguna categoría";
//JS MESSAGE
$TPLMSG["CONFIRM_PASSWORD"]="confirmar la contraseña";
//SITE MAP
$TPLMSG["SITEMAP"]="Mapa del Sitio";

//DELIVERY_NOTICE
$TPLMSG['DELIVERY_NOTICE'] = "entrega de notificación";
$TPLMSG['DELIVERY_ID'] = "ID del pedido";
$TPLMSG['DELIVERY_TOTAL_PRICE'] = "Precio de entrega";
$TPLMSG['DELIVERY_DATE'] = "fecha de entrega";

//VIRTUAL ACCOUNT
$TPLMSG['TRANSFER_BANK'] = "Transferencia Bancaria";
$TPLMSG['TRANSFER_BANK_CODE'] = "Nombre del banco";
$TPLMSG['TRANSFER_ACCOUNT'] = "Cuenta de Transferencia";

$TPLMSG['SELECT_COUNTRY'] = "Seleccione País";

$TPLMSG['EPAPER_REGISTERED'] = "your submit is registered!";
$TPLMSG['EPAPER_EMAIL_EXISTED'] = "e-mail existed!";
$TPLMSG['EPAPER_MISSING_DATA'] = "name or e-mail missing";
$TPLMSG['EPAPER_EMAIL_INVALID'] = "e-mail is invalid!";

$TPLMSG['SLIDE_BOX_IMAGE'] = "image";
$TPLMSG['SLIDE_BOX_OF'] = "of";
$TPLMSG['SLIDE_BOX_BTN_BACK'] = "Back";
$TPLMSG['SLIDE_BOX_BTN_NEXT'] = "Next";
$TPLMSG['SLIDE_BOX_BTN_PREV'] = "Prev";
$TPLMSG['SLIDE_BOX_CLICK_VIEW_LIST'] = "Click view images list";
$TPLMSG['SLIDE_BOX_CLICK_CLOSE_LIST'] = "Click close images list";

$TPLMSG['SUBMIT'] = "Submit";
$TPLMSG['COMPANY_ADDRESS'] = "Dirección";
$TPLMSG['PRODUCTS_FAVORITE'] = "Favorite";

$TPLMSG["APPLICATION"] = "Application";
$TPLMSG['PRODUCTS_SEARCH'] = "PRODUCTS SEARCH";
$TPLMSG['ENTER_KEYWORD'] = "Enter Keyword";
$TPLMSG['NEXT'] = "Próximo";
$TPLMSG['PREV'] = "Prev";
$TPLMSG['BACK_TO_LIST']="BACK TO LIST";
$TPLMSG['RELATED_PRODUCTS'] = "Related Products";
$TPLMSG["VIDEO"] = "Video";
$TPLMSG["ABOUT_US_CATE_DESC"] = "Company Information";
$TPLMSG["PRODUCTS_CATE_DESC"] = "Products Catalogue";
$TPLMSG["APPLICATION_CATE_DESC"] = "Products Appliction";
$TPLMSG["DOWNLOAD_CATE_DESC"] = "download";
$TPLMSG["NEWS_CATE_DESC"] = "Our News & Events";
$TPLMSG["VIDEO_CATE_DESC"] = "video";
$TPLMSG["FAQ_CATE_DESC"] = "faq";
$TPLMSG["SERVICE_CATE_DESC"] = "service";
$TPLMSG["CONTACT_US_CATE_DESC"] = "contact us";
$TPLMSG["SITEMAP_CATE_DESC"] = "sitemap";
$TPLMSG['CONTACT_US_ATTACH_FILES'] = "Attach files";
$TPLMSG["STORES"] = "Stores";
$TPLMSG["STORES_CATE_DESC"] = "Stores";
$TPLMSG['GUESTBOOK'] = "guestbook";
$TPLMSG['GUESTBOOK_CATE_DESC'] = "guestbook";
$TPLMSG["FACTORY"] = "Factory";
$TPLMSG['EBOOK_CATE_DESC'] = "E-catalog";
$TPLMSG['THUMB'] = "Thumb";
$TPLMSG['CART_QUICK_SHOPPING'] = "Quick Shopping";
$TPLMSG['CONTACT_S_1']="Sr.";
$TPLMSG['CONTACT_S_2']="Sra.";
$TPLMSG['CONTACT_S_3']="Srta.";
$TPLMSG['ACCOUNT_ACTIVATED'] = "Your account have been activated!!! Please login again.";
$TPLMSG['ACCOUNT_ACTIVATE_FAILED'] = "Account activating failed!";
$TPLMSG['REGISTER_NOTIFICATION'] = "Please click on the link in the mail send by system after you submit the info , which is process of activating your account.";
$TPLMSG['CONTACTUS_PRODUCT_LIST'] = "Product List";
$TPLMSG['CONTACTUS_POSITION'] = "Department & Position";
$TPLMSG['EPAPER_HEADER'] = "電子報頁首";
$TPLMSG['EPAPER_FOOTER'] = "電子報頁尾";
$TPLMSG['EPAPER_UNSUB_SUCCESS'] = "You've been remove from mail list";
$TPLMSG['GALLERY'] = "Gallery";
$TPLMSG["PAGE_NO_EXITS"] = "This Page doesn't exists!";
//Goodlink
$TPLMSG['GOODLINK'] = "Good link";
$TPLMSG['FIRST_TIME_SHOPPING'] = "First Time Shopping";
$TPLMSG['FIRST_S_BTN'] = "First Shopping?"; //第一次購物
$TPLMSG['FIRST_I_BTN'] = "First Inquiry?"; //第一次詢價
$TPLMSG['STORE_CATE_1'] = "real stores";
$TPLMSG['STORE_CATE_2'] = "web stores";
$TPLMSG['MEMBER_INFO'] = "Member Information"; //會員資訊
$TPLMSG['ADDRESSEE'] = "Addressee"; //收件人
$TPLMSG['SAM'] = "Same as Member"; //同會員資料
$TPLMSG['INVOICE_INFO'] = "Invoice Information"; //發票資訊
$TPLMSG['DUP_INVOICE'] = "Duplicate Uniform Invoice"; //二聯式發票
$TPLMSG['TRI_INVOICE'] = "Triplicate Uniform Invoice"; //三聯式發票
$TPLMSG['DONATE'] = "Donate"; //捐贈
$TPLMSG['VAT'] = "VAT number"; //統一編號
$TPLMSG['ARRIVAL_TIME'] = "Arrival time"; //到貨時間
$TPLMSG['YEAR'] = "Year"; //年
$TPLMSG['MONTH'] = "Month"; //月
$TPLMSG['DAY'] = "Day"; //日
$TPLMSG['NO_PAYMENT'] = "Please Select Payment Type!"; //請選擇付款方式
$TPLMSG["PLUS_FEE"] = "Fee"; //手續費
$TPLMSG["SHOPPING_RESULT"] = "Shopping Result"; 
$TPLMSG['AUTHORIZE_TERMINATE'] = "Authoriz terminated";
$TPLMSG['AUTHORIZE_FAILED'] = "Authoriz failed";
$TPLMSG['AUTHOZIED_FAILED_EXTEND_MSG'] = "<div class='auth_err_rc'>%s</div><div class='auth_err_msg'>Please confirm your credit card info！<br/>If you sure that your credit card is correct, contact with you issue bank. Thank you. </div>";
$TPLMSG['ORDER_SUCCESS'] = "Order Success";
$TPLMSG['ORDER_FAIL'] = "Order failed";

//歐付寶相關字彙
$TPLMSG["CREDIT"] = "Credit card"; //信用卡
$TPLMSG["WEBATM"] = "Web ATM"; //網路ATM
$TPLMSG["ATM"] = "ATM"; //自動櫃員機
$TPLMSG["CVS"] = "CVS"; //超商代碼 (需加收 30 元手續費)
$TPLMSG["BARCODE"] = "Barcode"; //超商條碼 (需加收 30 元手續費)
$TPLMSG["ALIPAY"] = "Alipay"; //支付寶
$TPLMSG["TENPAY"] = "Tenpay"; //財付通
$TPLMSG["TOPUPUSED"] = "Top-up"; //儲值消費
$TPLMSG['ALLPAY_BANK_CODE'] = "ATM Bank Code"; //ATM 銀行代碼
$TPLMSG['ALLPAY_VACCOUNT'] = "ATM Premium Account"; //ATM 付費帳號
$TPLMSG['ALLPAY_CVS_NO'] = "CVS code"; //超商繳費代碼
$TPLMSG['ALLPAY_CVS_BAR_1'] = "Barcode 1"; //超商繳費條碼 1
$TPLMSG['ALLPAY_CVS_BAR_2'] = "Barcode 2"; //超商繳費條碼 2
$TPLMSG['ALLPAY_CVS_BAR_3'] = "Barcode 3"; //超商繳費條碼 3
$TPLMSG['ALLPAY_EXPIRE'] = "Payment deadline"; //繳費期限
$TPLMSG['ORDER_PENDING'] = "Pending payment"; //待付款
$TPLMSG['ORDER_DONE'] = "Paid"; //已付款

/*周日名称*/
$TPLMSG['W_SUNDAY'] = "Sunday";
$TPLMSG['W_MONDAY'] = "Monday";
$TPLMSG['W_TUESDAY'] = "Tuesday";
$TPLMSG['W_WEDNESDAY'] = "Wednesday";
$TPLMSG['W_THRUSDAY'] = "Thursday";
$TPLMSG['W_FRIDAY'] = "Friday";
$TPLMSG['W_SATURDAY'] = "Saturday";
$TPLMSG['W_SUN'] = "Sun";
$TPLMSG['W_MON'] = "Mon";
$TPLMSG['W_TUE'] = "Tue";
$TPLMSG['W_WED'] = "Wed";
$TPLMSG['W_THR'] = "Thu";
$TPLMSG['W_FRI'] = "Fri";
$TPLMSG['W_SAT'] = "Sat";

/*新增語彙*/
/*以下是支付寶相關字彙*/
$TPLMSG["PAYMENT_ALI"] = "支付寶";
$TPLMSG["PAYMENT_TEN"] = "財付通";
$TPLMSG['ALI_FAIL_TITLE'] = "交易失敗";
$TPLMSG['ALI_OK_TITLE'] = "交易完成";
$TPLMSG["ALI_TITLE"] = "購物訊息";
$TPLMSG['ALI_FALE_MESSAGE'] = "產品售完或已下架";
$TPLMSG["ALI_SHIP_MSG"] = "請於支付寶平台確認運費";
$TPLMSG["MEMBER_ALI_QUERY"] = "查詢支付寶訂單";
$TPLMSG['SHIPPMENT_1'] = "臺灣本島";
$TPLMSG['SHIPPMENT_2'] = "離島地區(澎湖、金門、馬祖)";
$TPLMSG['SHIPPMENT_3'] = "中國大陸(支付寶)";
$TPLMSG['THANKS_FOR_ORDER'] = "大黑松小倆口感謝您的訂購!";
$TPLMSG['INVOICE_TYPE_2'] = "二聯式";
$TPLMSG['INVOICE_TYPE_3'] = "三聯式";
$TPLMSG['ORDER_BUYER_INFO'] = "訂購者資訊";
$TPLMSG['ORDER_RECI_INFO'] = "收件者資訊";
$TPLMSG['ORDER_INVOICE_INFO'] = "發票資訊";
$TPLMSG['DELIVER_STR'] = "配送日期及時段";
$TPLMSG['VAT_NUMBER'] = "統一編號";
$TPLMSG['PRODUCT_PRICE'] = "價格";
$TPLMSG['READ_MORE'] = "More";

//關閉網站
$TPLMSG['WEB_CLOSE_MSG'] = "Sorry, Our Website closed temporarily";

$TPLMSG['ATTACH'] = "Attach file";
$TPLMSG['MAX_UPLOAD_SIZE'] = "Max upload file size: %s";
$TPLMSG['COMMENT_NOTIFY_SUBJECT'] = "You got comment";
$TPLMSG['COMMENT_NOTIFICATION'] = 'You got comment in <a href="%1$s">%1$s</a>';
$TPLMSG['COMMENT_VALID_TAGS'] = "valid html tags: &lt;b&gt;, &lt;p&gt;, &lt;br&gt;, &lt;a&gt;";
$TPLMSG['COMMENT_GUEST'] = "Guest";
$TPLMSG['COMMENT_ADMIN'] = "Admin";
$TPLMSG['COMMENT_ROLE_WRAPPER1'] = "<span class='role'>%s</span>";
$TPLMSG['COMMENT_ROLE_WRAPPER2'] = "<span class='role-user'>%s</span>";
$TPLMSG['COMMENT_ROLE_WRAPPER3'] = "<span class='role-admin'>%s</span>";
$TPLMSG['COMMENT_ROLE'] = "Role: ".$TPLMSG['COMMENT_ROLE_WRAPPER1'];
$TPLMSG['COMMENT_FORM_VALID_MSG'] = "please fill data into fields with red border";
$TPLMSG['COMMENT_UPLOAD_DESCRIPTION'] = "You could upload images, but not required.";
$TPLMSG['COMMENT_LIST_TITLE'] = "Comment Title";
$TPLMSG['COMMENT_LIST_PUBLISH_DATE'] = "Date of publication";

$TPLMSG['MOBILE_VERSION'] = "Mobile Version";
$TPLMSG['COMPUTER_VERSION'] = "Desktop Version";
    
$TPLMSG['COPY_URL_DONE'] = "Replicated URL!";

$TPLMSG['STATUS_OFF'] = "off";
$TPLMSG['STATUS_ON'] = "on";
$TPLMSG['CLASSIFY_OTHER'] = "Other";
?>